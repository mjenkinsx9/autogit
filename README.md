...
| **Gemini CLI** | `skills/` (auto-discovered) | `gemini-extension.json` | ✅ real extension (skill); auto-ship hooks pending | [docs/gemini.md](docs/gemini.md) |
...
- **Gemini CLI** auto-discovers Agent Skills from `skills/<name>/SKILL.md`, so `gemini-extension.json` carries metadata only (no `skills` field) and the same skill is exposed natively. As with Codex/Cursor/Factory, the extension delivers the `/autogit` control surface; the automatic after-every-turn shipping (a turn-end hook) is the one piece not yet ported — see [docs/gemini.md](docs/gemini.md).
- **OpenCode** is an honest gap: it's a JS event module (no skills, no manifest), so a faithful port needs runtime-tested work that couldn't be validated here. [docs/opencode.md](docs/opencode.md) has a copy-pasteable reference plugin.
